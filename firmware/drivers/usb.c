...
// Estructura del paquete SETUP
typedef struct { uint8_t T; uint8_t R; uint16_t V; uint16_t I; uint16_t L; } __attribute__((packed)) SETUP;

// Descriptores originales (Alineados a 32-bits para la FIFO del ARM)
__attribute__((aligned(4))) const uint8_t DeviceDescriptor[] = {
    18,1,0,2,0,0,0,64,0x83,4,0x11,0x57,0,2,0,0,0,1
};
__attribute__((aligned(4))) const uint8_t ConfigDescriptor[] = {
    9,2,86,0,2,1,0,0x80,50,9,4,0,0,0,1,1,0,0,9,0x24,1,0,1,9,0,1,1,9,4,1,0,2,1,3,0,0,7,0x24,1,0,1,0x41,0,6,0x24,2,1,1,0,9,0x24,3,1,2,1,1,1,0,9,5,1,2,64,0,0,0,0,5,0x25,1,1,1,9,5,0x81,2,64,0,0,0,0,5,0x25,1,1,2
};
...
void USB_Write_EP0(const uint8_t *d, uint32_t l) {
    ...
    USB_DIEPCTL0 |= (1<<31)|(1<<26);
    for (uint32_t i=0; i<(l+3)/4; i++) {
        USB_DFIFO0 = *((uint32_t*)(d + i*4));
    }
}
...
// ============================================================================
// 3. LA MÁQUINA DE ESTADOS (IRQ)
// ============================================================================
void OTG_FS_IRQHandler(void) {
    uint32_t s = USB_GINTSTS;

    // Chivato de actividad USB
    GPIOC_BSRR = (1 << 29);
    ...
    if (s & (1<<4)) { // RX Data
        ...
        if ((p&0xF)==0 && ((p>>17)&0xF)==6) {
            ...
            if (st->R == 6) { // GET_DESCRIPTOR
                uint16_t req_len = st->L; // EL PARCHE: Leer cuánto pide el PC
                if (st->V>>8 == 1) {
                    USB_Write_EP0(DeviceDescriptor, (req_len < 18) ? req_len : 18);
                }
                else if (st->V>>8 == 2) {
                    USB_Write_EP0(ConfigDescriptor, (req_len < 86) ? req_len : 86);
                }
            } else if (st->R == 5) { // SET_ADDRESS
                USB_DCFG = (USB_DCFG & ~0x7F0) | (st->V << 4);
                USB_Write_EP0(0,0);
            } else if (st->R == 9) { // SET_CONFIGURATION
                USB_DAINTMSK |= 0x20002;
                USB_DIEPCTL1 = 0xC4080000;
                USB_Write_EP0(0,0);
            }
        } else {
            uint32_t bcnt = (p >> 4) & 0x7FF;
